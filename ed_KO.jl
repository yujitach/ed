	...
	for preind = 1 : len
		...
		for i = 1 : L
			...
			if sp==sXX  && isρ1ρ(flag,ind,i)
				C[newPreind(state,i,sPM)] -= ξ * y1 * B[preind]
				C[newPreind(state,i,sMP)] -= ξ * y2 * B[preind]
				C[newPreind(state,i,s00)] -= ξ * x * B[preind]
			elseif sp==sPM
				C[newPreind(state,i,sXX)] -= y1 * ξ * B[preind]
				C[newPreind(state,i,sMP)] -= y1 * y2 * B[preind]
				C[newPreind(state,i,s00)] -= y1 * x * B[preind]
			elseif sp==sMP
				C[newPreind(state,i,sXX)] -= y2 * ξ * B[preind]
				C[newPreind(state,i,sPM)] -= y2 * y1 * B[preind]
				C[newPreind(state,i,s00)] -= y2 * x * B[preind]
			elseif sp==s00
				C[newPreind(state,i,sXX)] -= x * ξ * B[preind]
				C[newPreind(state,i,sPM)] -= x * y1 * B[preind]
				C[newPreind(state,i,sMP)] -= x * y2 * B[preind]
			elseif sp==s0P
				C[newPreind(state,i,sP0)] -= y1 * y2 * B[preind]
				C[newPreind(state,i,sMM)] -= y1 * z * B[preind]
			elseif sp==sP0
				C[newPreind(state,i,s0P)] -= y2 * y1 * B[preind]
				C[newPreind(state,i,sMM)] -= y2 * z * B[preind]
			elseif sp==sMM
				C[newPreind(state,i,s0P)] -= z * y1 * B[preind]
				C[newPreind(state,i,sP0)] -= z * y2 * B[preind]
			elseif sp==s0M
				C[newPreind(state,i,sM0)] -= y2 * y1 * B[preind]
				C[newPreind(state,i,sPP)] -= y2 * z * B[preind]
			elseif sp==sM0
				C[newPreind(state,i,s0M)] -= y1 * y2 * B[preind]
				C[newPreind(state,i,sPP)] -= y1 * z * B[preind]
			elseif sp==sPP
				C[newPreind(state,i,s0M)] -= z * y2 * B[preind]
				C[newPreind(state,i,sM0)] -= z * y1 * B[preind]				
			end
		end
	end
end
